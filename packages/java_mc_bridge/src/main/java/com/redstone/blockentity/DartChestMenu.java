...
/**
 * Container menu for chest-like block entities with grid-based inventory.
 *
 * This menu supports:
 * - Dynamic grid layout (rows × columns)
 * - ContainerData synchronization for custom values
 * - Standard player inventory integration
 *
 * Unlike DartBlockEntityMenu (furnace-style with 3 fixed slots), this menu
 * creates a grid of slots like a chest, suitable for block entities with
 * larger inventories.
 */
public class DartChestMenu extends AbstractContainerMenu {
    private static final Logger LOGGER = LoggerFactory.getLogger("DartChestMenu");
     ...
     */
    public DartChestMenu(int containerId, Inventory playerInventory) {
        this(containerId, playerInventory, 3, 9);
    }

    /**
     * Client-side constructor with custom dimensions.
     */
    public DartChestMenu(int containerId, Inventory playerInventory, int rows, int columns) {
        this(containerId, playerInventory,
             new SimpleContainer(rows * columns),
     ...
     */
    public DartChestMenu(int containerId, Inventory playerInventory, Container container,
                         ContainerData data, int rows, int columns) {
        super(RedstoneMenuTypes.DART_CHEST_MENU, containerId);

        this.rows = rows;
        this.columns = columns;
        this.containerSlotCount = rows * columns;

        checkContainerSize(container, containerSlotCount);
        ...
        // Standard slot is 18x18 pixels, image width is 176 pixels
        int imageWidth = 176;
        ...
        LOGGER.debug("DartChestMenu created: containerId={}, rows={}, columns={}, slots={}",
                     containerId, rows, columns, containerSlotCount);
    }
    ...
    /**
     * Get the container slot count (rows × columns).
     */
    ...
    public void clicked(int slotIndex, int button, ClickType clickType, Player player) {
        // Delegate click handling to Dart if initialized
        if (DartBridge.isInitialized() && slotIndex >= 0 && slotIndex < containerSlotCount) {
            // Could add Dart click callbacks here if needed
        }
