---
title: Players
description: Interact with players - messages, inventory, effects
icon: User
---
...
player.sendTitle('§6Welcome!', subtitle: 'Enjoy your stay');
...
```dart
// Teleport to coordinates (using BlockPos)
player.teleport(BlockPos(100, 64, 200));

// Teleport with precise positioning (using Vec3)
player.teleportPrecise(Vec3(100.0, 64.0, 200.0));
...
player.experienceLevel = 30;       // Set level directly
```
...
player.position       // Vec3 position (use player.position.x, .y, .z)
...
Events.onPlayerJoin((player) {
  ...
  player.sendTitle('§6Welcome!', subtitle: '§7Here are some items');
