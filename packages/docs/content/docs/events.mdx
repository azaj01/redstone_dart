---
title: Events
description: React to game events and player actions
icon: Zap
---
...
**Game Events:**
- `Events.addTickListener((int tick) { })` — Every game tick (20/second)

### The Tick Event

`addTickListener` runs 20 times per second. It's powerful but dangerous—expensive code here causes lag for everyone.

```dart
Events.addTickListener((int tick) {
  // tick is the current game tick count
